...
�random� mayores a 10 y menores a 155 (incluidos ambos). La carga finaliza con 
el valor 20.
b. Un m�dulo no recursivo que reciba el vector generado en a) e imprima el
contenido del vector.
c. Un m�dulo recursivo que reciba el vector generado en a) e imprima el contenido
del vector.
d. Un m�dulo recursivo que reciba el vector generado en a) y devuelva la suma de 
los valores pares contenidos en el vector.
e. Un m�dulo recursivo que reciba el vector generado en a) y devuelva el m�ximo 
valor del vector.
...
g. Un m�dulo que reciba el vector generado en a) e imprima, para cada n�mero 
contenido en el vector, sus d�gitos en el orden en que aparecen en el n�mero. 
...
procedure ImprimirVectorRecursivo (v: vector; dimL: integer);
    procedure Imprimir (v: vector; dimL,i: integer);
    begin    
        if ( i <= dimL)then
        begin
            writeln(v[i], ' | ');
...
function ObtenerMaximo (v:vector;dimL:integer):integer;
    ...
    function  Max (v: vector;i,maxi,dimL: integer): integer;
    begin
        if ( i<= dimL)then
        begin
            if ( maxi < v[i])then maxi:=v[i];
            Max:= Max(v,i+1,maxi,diml);
        end
        else Max:=maxi;
    end;  
var
...
begin   
    i:=1; maxi:=-1;
    ObtenerMaximo:=Max(v,i,maxi,diml);
end;   
     

function  BuscarValor (v: vector; dimL, valor: integer): boolean;
        procedure Buscar(v: vector; dimL,i, valor: integer;var ok:boolean);
        begin
            if ( i<= dimL)then
            begin
                if ( v[i] <> valor)then Buscar(v,diml,i+1,valor,ok)
                else
                    ok:=true;
            end;
        end;
